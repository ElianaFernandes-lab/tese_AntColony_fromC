...
//TIME
	...
	runCplexLR(tLR,scal_param,dat, a);
	cout << endl<<"FINISHED COMPUTING CPLEX LR"<<endl;
#endif
#ifdef HISTORY
	myfile.open ("history1.txt",ios::app);
	myfile<<"CPLEX LR for scal param COMPUTED"<<endl;
	myfile << setw(12) << fichIn << endl;
	myfile.close();
#endif
#ifdef TIME
...
#endif
#ifndef REP
	prepareOutputFile(fichIn, fichOut, scal_param,tLR);
#endif
					...
					for(i=0;i<dat.nbNodes;i++)
						for(j=0;j<dat.nbNodes;j++){
							if(ants[0].avail_tau[i][j][p]>0){
								myfile << "ants.avail_tau"<<"["<<i<<"]["<<j<<"]["<<p<<"] = "<< ants[0].avail_tau[i][j][p] << endl;
								av_sol++;
							}
							tot_sol++;
						}
...
//NEW
...
#ifdef HISTORY
			myfile.open ("history1.txt",ios::app);
			myfile<<endl<<"BEST ITERATION :"<<it_best<<" COST = "<<better.cost;
			myfile.close();
#endif
#ifndef USE_CPLEX
			//TIME
			...
			OutputFile(it, it_best, fichOut, gp.cost , gp.time, better.cost, better.time, seconds, gp.value); 
			}else{
				cout << "NO SOLUTION FOUND" << endl;
			}
			// write log file
