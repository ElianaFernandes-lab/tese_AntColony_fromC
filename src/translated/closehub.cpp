...
int CloseHub(data dados, ant& ants, iteration& iter, double max_cost){

#ifdef CLOSEHIST
    ofstream myfile;
    myfile.open ("CLOSEhistory.txt",ios::app);
    ...
    if(ants.life>0){
		...
		// make a temporary copy of the hub list, so that the original list is not lost
		int* temp_list_hubs;
		//memory allocation
		temp_list_hubs = new (nothrow) int [dados.nbNodes];
		if (temp_list_hubs == 0)
			cout<<"Error: memory could not be allocated\n";
        ...
        do{
            ...
            if(min_connects==1){
                ...
                if(nr_hubs>0){
                    ...
                    } while(l<nr_hubs); //end  do 2
                    // put the rest of the list to -1
                    for(l=nr_hubs+1;l<nr.phubs[prod];l++)
                        temp_list_hubs[l]=-1;
#ifdef CLOSEHIST
                    myfile.open ("CLOSEhistory.txt",ios::app);
                    ...
                    for(l=0;l<nr_hubs;l++){
						...
						// o hub está ligado a si mesmo, nao ha transfer costs mas ponho no daso de se ter d_jj!=0
						cost_subtract = hub_cost[prod][hub]+dados.d[hub][hub]*(dados.chi[prod]*dados.O[hub][prod]+dados.delta[prod]*dados.D[hub][prod]); 
#ifdef CLOSEHIST
                        ofstream myfile;
                        myfile.open ("CLOSEhistory.txt",ios::app);
...
#endif
                flag=2;
            } // else if(min_connects==1)
			
        }while (flag<1); //end do1
